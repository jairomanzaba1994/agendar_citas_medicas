        ...
        <div class="card-header container">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 text-center text-md-left pb-3">
                    <a class="btn btn-primary" href="{{ route('doctors.create') }}"><b>Agregar doctor</b></a>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 text-center text-md-right">
                    <form action="{{ route('doctors.index') }}" method="GET">
                        <div class="mb-3 row">
                            <div class="col-md-10 text-md-right pb-1">
                                <input type="text" name="filterValue" id="filterValue" placeholder="Buscar por nombre del doctor" class="form-control rounded border-primary text-secondary">
                            </div>
                ...
                <tbody>
                    @foreach ($doctors as $doctor)
                        <tr>
                            ...
                            <td width="2px"><a href="{{ route('doctors.show', $doctor->id) }}"
                                class="btn btn-primary btn-sm mb-2">Mostrar</a></td>

                            <td width="5px"><a href="{{ route('doctors.edit', $doctor->id) }}"
                                class="btn btn-primary btn-sm mb-2">Editar</a></td>

                            <td width="5px">
                                <form action="{{ route('doctors.destroy', $doctor->id) }}" method="POST" onsubmit="return confirmarEliminar(event)">
                                    @csrf
                                    @method('DELETE')
